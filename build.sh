...
"$KOTLIN_SHARE_DIR/gradlew" -p "$KOTLIN_SHARE_DIR/moduleA" podspec
"$KOTLIN_SHARE_DIR/gradlew" -p "$KOTLIN_SHARE_DIR/moduleB" podspec
"$KOTLIN_SHARE_DIR/gradlew" -p "$KOTLIN_SHARE_DIR/moduleC" podspec

# Run CocoaPods to configure the Xcode project.
pod --project-directory="$IOS_DIR" install

# Run Xcode to build the app.
xcodebuild -sdk iphonesimulator -arch x86_64 -configuration Release -workspace "$IOS_DIR/FooTest.xcworkspace" -scheme 'FooTest (iOS)'
